...
if __name__ == "__main__":
    ...
    # Read Map 閾值操作：將所有像素值大於 128 的設置為 255（白色），小於或等於 128 的設置為 0（黑色），從而達到二值化效果。
    img[img>128] = 255    
    img[img<=128] = 0
    m = np.asarray(img)
    ...
    if args.planner == "a_star":
        ...
        path = planner.planning(start, goal, img=img)
    elif args.planner == "rrt_sharp":
        from PathPlanning.rrt_sharp import PlannerRRTSharp as Planner
        planner = Planner(m, 30)
        path = planner.planning(start, goal, img=img)
    else:
