...
class PolicyNet(nn.Module):
	...
	def __init__(self, s_dim, a_dim, std=0.5):
		...
		#可以用init_(self.fc1)，這裡 lambda m: ... 說的是：「給我一個層 m，我就用 init 函數來初始化它。」
		init_ = lambda m: init(
			m,
			nn.init.orthogonal_,
			lambda x: nn.init.constant_(x, 0),#bias =0
		...
		self.main = nn.Sequential(
            init_(nn.Linear(s_dim, 64)),  # 第一層：s_dim -> 64 
            nn.ReLU(),
            init_(nn.Linear(64, 64)),     # 第二層：64 -> 64
            nn.ReLU()
        )
...
class ValueNet(nn.Module):
	...
	def forward(self, state):
		return self.main(state)[:, 0] #所有的列的第一元素
